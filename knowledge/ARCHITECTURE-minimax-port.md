# The MiniMax Design port (ARCHIVED REFERENCE — not the live pipeline)

> This describes the media-agent → router/planner/executor → `plan.py` / `gen.py` → canvas-node path
> ported verbatim from MiniMax Design. **No production in this repo has ever run through it.**
> Live productions run `tools/run_crun.py`, `tools/santan_build2.py`, `tools/post.py` and
> `tools/ledger.py` directly. Read this only when working on that ported path.
> Moved out of CLAUDE.md on 23 Sep 2026 (system audit).
